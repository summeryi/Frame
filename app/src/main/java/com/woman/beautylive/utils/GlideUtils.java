package com.woman.beautylive.utils;
...
public class GlideUtils implements cn.finalteam.galleryfinal.ImageLoader{
    ...
    public static void displayImageView(ImageView mImageView,String imageUrl,Drawable error){
        Glide.with(mImageView.getContext()).load(imageUrl).error(error).into(mImageView);
    }



    @BindingAdapter({"imageCircleUrl","imageError"})
    public static void displayCircleImageView(ImageView mImageView,String imageUrl,Drawable error){
        Glide.with(mImageView.getContext()).load(imageUrl).error(error).transform(new GlideCircleTransform(mImageView.getContext())).into(mImageView);
    }
    ...
    //默认加载
    public static void loadRoundImageView(Context mContext, String path, ImageView mImageView, int lodingImage, int errorImageView) {
    ...
    //设置跳过内存缓存加载中以及加载失败图片并且指定大小
    public static void loadImageViewLodingSizeCache(Context mContext, String path, int width, int height, ImageView mImageView, int lodingImage, int errorImageView) {
        Glide.with(mContext).load(path).override(width, height).placeholder(lodingImage).skipMemoryCache(true).error(errorImageView).into(mImageView);
    }
